...
const logger = createLogger("ApiCollectionsPageServer");
...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  if (!accessToken) {
    logger.warn("No access token available for API collections page");
    return {
      collections: [],
      hasApiAccess: false,
  ...
  try {
    logger.info("=== FETCHING MY API COLLECTIONS ===");
