...
const logger = createLogger("EditApiCollectionPageServer");
...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  // No special roles required for editing personal API collections
  const requiredRoles: any[] = [];

  if (!accessToken) {
    logger.warn("No access token available for edit API collection page");
    return {
      collection: null,
      userEntitlements,
      requiredRoles,
  ...
  try {
    logger.info("=== FETCHING API COLLECTION FOR EDIT ===");
