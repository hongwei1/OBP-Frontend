...
const logger = createLogger("LockUserPageServer");
...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  if (!accessToken) {
    logger.warn("No access token available for lock user API call");
    return {
      user: null,
      user_id,
      hasApiAccess: false,
...
export const actions = {
  lockUser: async (event: RequestEvent) => {
    const session = event.locals.session;

    if (!session?.data?.user) {
      return fail(401, { error: "Unauthorized", action: "lockUser" });
    }
    ...
    if (!token) {
      return fail(401, { error: "Unauthorized: No access token found in session.", action: "lockUser" });
    }

    const { user_id } = event.params;

    if (!user_id) {
      return fail(400, { error: "User ID is required.", action: "lockUser" });
    }
    ...
    try {
      ...
      logger.error("Error fetching user for lock:", e);
      const { message } = extractErrorDetails(e);
      return fail(500, { error: message, action: "lockUser" });
    }

    if (!user?.provider || !user?.username) {
      return fail(400, { error: "Could not determine provider/username for this user.", action: "lockUser" });
    }

    try {
      const lockEndpoint = `/obp/v6.0.0/users/${encodeURIComponent(user.provider)}/${encodeURIComponent(user.username)}/locks`;
      const result = await obp_requests.post(lockEndpoint, {}, token);

      logger.info(`User ${user.username} locked successfully`, result);
