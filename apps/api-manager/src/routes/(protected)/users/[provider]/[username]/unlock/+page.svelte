...
<svelte:head>
  <title>Unlock User - {data.username} - API Manager II</title>
</svelte:head>
...
<div class="container mx-auto px-4 py-8">
  <nav class="breadcrumb mb-6">
    ...
    <a href="/users/{data.provider}/{data.username}" class="breadcrumb-link">{data.username}</a>
    <span class="breadcrumb-separator">›</span>
    <span class="breadcrumb-current">Unlock User</span>
  </nav>
    ...
    <div class="panel mb-6">
      <div class="panel-header">
        <div class="flex items-center justify-between">
          <div>
            <h2 class="panel-title">Unlock User</h2>
            <div class="panel-subtitle">Reset the lock status and allow the user to log in again</div>
          </div>
          <form method="POST" action="?/unlockUser" use:enhance={handleEnhance}>
...
<style>
  ...
  :global([data-mode="dark"]) .response-data {
