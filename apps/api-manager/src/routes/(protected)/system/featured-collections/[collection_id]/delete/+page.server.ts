...
const logger = createLogger("DeleteFeaturedCollectionPageServer");
...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  if (!accessToken) {
    logger.warn(
      "No access token available for delete featured collection page",
    );
    return {
      featured: null,
      collection: null,
  ...
  try {
    // Fetch featured collections to find the one we're deleting
